...
def manual_url(movie_df): 
    ...
    for idx, row in um.iterrows():
        ...
        if response.status_code == 200: 
            ...
            if row['Year'] is not None and year is not None  and row['Year']-2<=year<=row['Year']: 
                um.loc[idx, 'Check']='OK'
...
def manual_url_year1(movie_df): 
    ...
    for idx, row in um.iterrows():
        if row['Check']!='OK':
            row['url']='https://letterboxd.com/film/'+row["movie"]+'-'+row['year1']+'/'
            try: 
                headers = {'User-Agent': 'Mozilla/5.0'}
                response=requests.get(row['url'], headers=headers, timeout=10)
                response.raise_for_status()
...
#movie_wy1=manual_url_year1(df_Movie)


#movie_wy1.to_excel("Manual_Check1.xlsx",index=False)
...
def manual_url_year2(movie_df): 
    ...
    for idx, row in um.iterrows():
        if row['Check']!='OK':
            row['url']='https://letterboxd.com/film/'+row["movie"]+'-'+row['year2']+'/'
            try: 
                headers = {'User-Agent': 'Mozilla/5.0'}
                response=requests.get(row['url'], headers=headers, timeout=10)
                response.raise_for_status()
