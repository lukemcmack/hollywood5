...
def get_unique_movies(csv_path):
    ...
    print(
        f"Found {len(df)} total movies, {len(df_unique)} unique after removing duplicates"
    )
    ...
    for rating in rating_values:
        ...
        for page in range(1, 257):  # Pages 1-256
            ...
            for review in review_elements:
                ...
                parsed_rating = (
                    parse_rating(rating_element.text.strip())
                    if rating_element
                    else None
                )
                ...
                date = (
                    ...
                    if time_element
                    else (date_element.text.strip() if date_element else "No date")
                )

                liked = (
                    review.find("span", class_="has-icon icon-16 icon-liked")
                    is not None
                )
