...
def build_features(df, Year, words):
    ...
    train_df.loc[:, "clean"] = train_df["clean"].apply(
        lambda x: popular_words(x, common_words)
    )
    ...
    test_df.loc[:, "clean"] = test_df["clean"].apply(
        lambda x: popular_words(x, common_words)
    )
    ...
    train_joined = pd.concat([train_df, train_vectors], axis=1).drop(
        ["Review Text", "clean"], axis=1
    )
    test_joined = pd.concat([test_df, test_vectors], axis=1).drop(
        ["Review Text", "clean"], axis=1
    )
