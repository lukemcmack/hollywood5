...
def generate_slug_variations(base_slug, year):
    ...
    if pd.isna(year) or not str(year).isdigit():
        return [base_slug]  # Just try base slug if year is invalid
    
    year = int(year)
    variations = []
    
    # 1. Exact year with hyphen (most common)
    variations.append(f"{base_slug}-{year}")

    # 2. Nearby years (±1-2 years)
    for i in range(1, 3):
        variations.extend([
            f"{base_slug}-{year+i}",
            f"{base_slug}-{year-i}"
    ...
    # 3. Without year (for unique titles)
...
def process_movies(input_csv, output_csv, title_col='Movie Title', year_col='Year', workers=10):
    """Process all movies in parallel"""
    # Read CSV with proper NA handling
    df = pd.read_csv(input_csv, keep_default_na=True, na_values=[''])
    
    # Clean data - convert all titles to strings
